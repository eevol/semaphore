...
function installHostKeys (task, playbook, done) {
	...
	playbookOutputHandler.call(task, "Updating SSH Keys in " + home + "\n");
...
function playTheBook (task, playbook, done) {
	...
	// always connect as user 'ansible' via SSH
	args.push('-u ansible');
	...
	// the playbook file
	var splitIndex = task.job.play_file.indexOf(" ");
	if (splitIndex > -1) {
		playbookOutputHandler.call(task, "\nEnabling extra vars");
		args.push('-e');
		args.push(task.job.play_file.substring(splitIndex + 1));
		args.push(task.job.play_file.substring(0, splitIndex));
	} else {
		args.push(task.job.play_file);
	}

	playbookOutputHandler.call(task, "\nCalling ansible-playbook with args "+JSON.stringify(args)+".\n");
	// the extra vars
	if(task.job.extra_vars.length>0){
